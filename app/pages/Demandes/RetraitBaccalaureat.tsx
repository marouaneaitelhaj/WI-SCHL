...
export default function RetraitBaccalaureat() {
  ...
  return (
    <View className="space-y-5 flex items-center w-full">
      ...
      >
        <Checkbox.Item onPress={() => setCheckedNumber(1)}
          status={checkedNumber == 1 ? "checked" : "unchecked"}
          label="Retrait temporaire"
        />
        <Checkbox.Item onPress={() => setCheckedNumber(2)} status={checkedNumber == 2 ? "checked" : "unchecked"} label="Retrait définitif" />
        <Pressable className="bg-[#5156BE] my-2 p-2 rounded-md w-[100%] justify-center items-center" onPress={() => {
          setShowModal(false);
          Alert.alert("Êtes-vous sûr de vouloir créer cette demande ?", "Êtes-vous sûr de vouloir créer cette demande ?", [
            {
              text: "Annuler",
              style: "cancel",
            },
